...
)
{
	...
	if (_optimiserSettings.runYulOptimiser && _localVariables.empty())
	{
		...
		if (_system)
		{
			...
			solAssert(m_generatedYulUtilityCode.empty(), "");
			m_generatedYulUtilityCode = yul::AsmPrinter(yul::AsmPrinter::Mode::OmitDefaultType, dialect)(*obj.code);
			std::string code = yul::AsmPrinter{yul::AsmPrinter::Mode::OmitDefaultType, dialect}(*obj.code);
